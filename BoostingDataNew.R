	...
	# subTrain2<- cbind(subTrain2,date_fea$month)
...
# competitors 
 ...
 # index_feature_competitor <- grep('comp',colnames(subTrain2))
 # competitor <- subTrain2[,index_feature_competitor]

 # index_feature_comp_percent_diff <- grep('percent_diff',colnames(competitor ) )
 # index_feature_comp_rate <- setdiff(grep('rate',colnames(competitor)), index_feature_comp_percent_diff)
 # index_feature_comp_inv <-setdiff(c(1:ncol(competitor)),c(index_feature_comp_percent_diff,index_feature_comp_rate))


 # index_all_missing<-which(apply(competitor,1,function(x) all(is.na(x))))
 # index_partial_missing <- setdiff(c(1:nrow(competitor)),index_all_missing)

 #--> treat hotels of all comp data missing with worst case, since hotels are not good if they only put 
 #ads on expedia

 # competitor[index_all_missing,index_feature_comp_rate] <- 0
 # competitor[index_all_missing,index_feature_comp_inv] <- 0
 # competitor[index_all_missing,index_feature_comp_percent_diff] <- 0 #mean(data.matrix(competitor[,index_feature_comp_percent_diff]),na.rm=T)

 #-->treat hotels of partial data missing with its  mean of available value
 # competitor[index_partial_missing, index_feature_comp_rate] <- t(apply(data.matrix(competitor[index_partial_missing, index_feature_comp_rate]),
		# 						  1,mean_comp ))

 # competitor[index_partial_missing, index_feature_comp_inv] <-t( apply(data.matrix(competitor[index_partial_missing, index_feature_comp_inv]),
                  #                                                  1,mean_comp))

 # competitor[index_partial_missing, index_feature_comp_percent_diff] <-t( apply(data.matrix(competitor[index_partial_missing, index_feature_comp_percent_diff]),
 #                                                                    1, mean_comp))

 # subTrain2[,index_feature_competitor] <- competitor
...
#    save(validation_data,file='validation_data.RData')
  #    save(train_data,file='train_data.RData')
      write.svmlight(labels[c(train_index,test_index)],data.matrix(XData[c(train_index,test_index),]),file='train_data1')
