...
sofia.formula <- function(x, data
  ...
  , lambda = 0.1 
  , iterations = 100000
  , learner_type = c("pegasos", "sgd-svm", "passive-aggressive", "margin-perceptron", "romma", "logreg-pegasos","logreg") 
  ...
  if(!(learner_type %in% c("pegasos", "sgd-svm", "passive-aggressive", "margin-perceptron", "romma", "logreg-pegasos","logreg")))
  ...
  if (colnames(parsed$data)[1]=='qid')
  {
    dimensionality <- ncol(parsed$data)
  }
  else
  {
...
sofia.fit <- function(x, y
  ...
  names(weights) <- c("(Offset)", setdiff(colnames_,'qid')) 
