...
# main function
main_svm_rank<-function(fold=1,train_portion=0.5,testcode=0,traincode=0){
  load('train.RData')
  ...
  for (i in 1:nrow(XData))
  {
    XData_sim[i,]<-as.vector(XData[i,names_query_index]%o%                              
                               c(polyFea(XData[i,names_prop_index],1))
    )
  }
...
# java -jar RankLib.jar -train svm_light_train_data  -ranker 6 -validate svm_light_test_data -metric2t NDCG@38 -save mymodel.txt  
