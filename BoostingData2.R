...
main_boosting_data<-function(fold=1,train_portion=0.5,testcode=0,traincode=0){
  ...
  subTrain2$price_usd<-log( subTrain2$price_usd+1e-8)
  ...
  subTrain2$visitor_hist_adr_usd<-log( subTrain2$visitor_hist_adr_usd+1e-8)
  ...
  xTemp<-local_scale(XData[,names_numeric],srch_id)

  print('local_scale finished')
  browser()

  XData<-cbind(XData_Dis,XData_whole,xTemp)
#   XData_sim<-matrix(nrow=nrow(XData),ncol=(ncol(xTemp))*ncol(xTemp))
#   for (i in 1:nrow(XData))
#   {
#     XData_sim[i,]<-as.vector(                             
#       c(polyFea(xTemp[i,],2))
#     )
#   }
  
#   XData_sim<-vector('list',length=ncol(xTemp))
#   for (i in 1:nrow(XData))
#   {
#     XData_sim[[i]]<-c(polyFea(xTemp[i,],2))
# 
#   }
#   XData_sim<- matrix(unlist(XData_sim),byrow=T,ncol=(ncol(xTemp))*ncol(xTemp))

  
#   XData_sim<-matrix(0,nrow=nrow(XData),ncol=(ncol(xTemp))*ncol(xTemp))
#   
#   XData_sim <-.C('Poly2',as.double(t(xTemp)),as.double(XData_sim),
#                                    as.integer(nrow(xTemp)),as.integer(ncol(xTemp)))[[2]]
#   XData_sim<-matrix(XData_sim,byrow=T,ncol=(ncol(xTemp))*ncol(xTemp))  
# 
# 
#   
#   rm(xTemp)
#   XData<-cbind(XData,XData_sim)
  ...
  if (testcode==0)
  {
    for (i in 1:fold)
    {
      ...
      validation_index<-unlist(validation_index[1:num_validate])
      test_index<-unlist(test_index[1:num_test])
      ...
      write.svmlight(labels[train_index],data.matrix(XData[train_index,]),file='train_data2')
      write.svmlight(labels[test_index],data.matrix(XData[test_index,]),file='validation_final_data2')
      write.svmlight(labels[validation_index],data.matrix(XData[validation_index,]),file='validation_data2')
